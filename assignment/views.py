        ...
        if form.is_valid():
            ...
            if e in ['.pdf', 'txt']:
                assignment = Assignment.objects.create(pdf_file=pdf_file,
                            index=o_t.user.index_number, status='submitted', q_number=rn, date_submitted=date, date_created=timezone.now(), user=o_t.user, studentothercourse=o_t, uploadedfile=u, course=sel.courses)
...
def student_result(request, pk, a_pk, model_class=StudentResult, form_class=StudentResultForm):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid:
            ...
            res = StudentResult.objects.create(q_number=ass.q_number, title=obj.title, status='graded', scored=obj.scored,
...
class GradeUpdateView(UpdateView):
    model = StudentResult
    fields = ['title', 'scored', 'total', 'marker']
...
class DownloadPDF(View):
	def get(self, request, *args, **kwargs):
		pdf = render_to_pdf('assignment/pdf_template.html', {'st':StudentOtherCourse.objects.filter(choose_course=self.kwargs['c']), 'sts':StudentOtherCourse.objects.filter(choose_course=self.kwargs['c']).first(), 'semester': Semester.objects.first(), 'c':self.kwargs['c']})
